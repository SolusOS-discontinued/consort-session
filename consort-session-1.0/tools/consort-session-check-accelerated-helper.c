/* gcc -o consort-session-accelerated `pkg-config --cflags --libs xcomposite gl` -Wall consort-session-is-accelerated.c */
 ...
 *    consort-session to directly try and do any GL detection; better to
 *    use a helper binary.
 *
...
{
        fprintf (stderr, "consort-session-is-accelerated: %s\n", str);
}
